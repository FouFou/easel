...
//	$wp_customize->add_section('easel-background-colors' , array('title' => __('Background Colors','easel')));
	...
	foreach ($css_array as $setinfo) {
		...
		$wp_customize->add_setting($setinfo_register_name, array('sanitize_callback' => 'sanitize_hex_color_no_hash', 'sanitize_js_callback' => 'maybe_hash_hex_color'));
	...
	$wp_customize->add_control( 'easel-customize-checkbox-rounded', array(
	...
	if (function_exists('ceo_pluginfo')) {
		...
		$wp_customize->add_control( 'easel-customize-comic-in-column', array(
